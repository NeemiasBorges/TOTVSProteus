...
// Definições para indexar os elementos do Array aCount - Rateio de Impostos
#DEFINE idxIRR 1
#DEFINE idxPIS 2
#DEFINE idxCOF 3
#DEFINE idxCSL 4 
...
If SF1->( FieldPos( "F1_GF" ) ) > 0
	aCores	:= {	{'Empty(F1_STATUS).And.F1_GF<>"GF"'	,'ENABLE'			},;	// NF Nao Classificada
					{'Empty(F1_STATUS).And.F1_GF=="GF"'	,'BR_MARRON_OCEAN'	},;	// NF Não Classificada com Guarda Fiscal
...
Local aColTrbGen    := {}
Local aParcTrGen    := {}
Local cIdsTrGen		:= ""//variável que vai guardar os ids de tributos genéricos.
...
Local lCtbDtBl	 := IIf((SuperGetMV("MV_CTBDTBL",.F.,1)) == 1,.T.,.F.) // 1 = data digitação >1 = data base
...
PRIVATE lContDCL   := .T.
Private cAliasTPZ	:= IIf( lExiTmpTb, GetNextAlias(),  oTempTable:GetAlias() ) // Utilizado na função NGGARANSD1 que está no X3_VALID do campo D1_GARANTI
...
If lRet
	...
	lAbreTela  := lWhenGet //Determina se no final da execauto é aberta a tela do docto de entrada para edição do usuário.
	...
	If lContinua .And. ( l103Inclui .Or. l103Exclui .Or. l103Class )
		If lCtbDtBl .And. (l103Exclui .Or. ( l103Class .And. SF1->F1_STATUS <> "C"))
			...
			aNFEletr  := {CriaVar("F1_NFELETR"),CriaVar("F1_CODNFE"),CriaVar("F1_EMINFE"),CriaVar("F1_HORNFE"),CriaVar("F1_CREDNFE"),CriaVar("F1_NUMRPS"),;
				...
				If l103Auto
					...
					IF aScan(aAutoCab,{|x| AllTrim(Upper(x[1]))=="F1_SERIE"}) == 0
						Help(" ",1,"CPOobrigat",,STR0574,1,0)//"obrigatório informar o campo série para as notas com formulário próprio = 'S' "
						lContinua := .f.
					Endif	
				EndIf
		Else
	...
	If lContinua
		...
		ElseIf l103Exclui
			...
			If lContinua .And. (lIntWMS .Or. lWmsCRD) .And. SF1->F1_TIPO $ "N|D|B" //-- Validação se pode excluir a nota fiscal pelo WMS
				...
				DbSetOrder(1)
					...
					aStruSD1  := SD1->(dbStruct())
						...
						if SF1->F1_FORMUL == "S"
							...
							cQuery    += "   AND SD1.D1_FORMUL	in ( '"+SF1->F1_FORMUL+"',' ')"
						endif
	...
	if lContinua .and. cTipo == "N" .and. l103Exclui .and. len(aCols) > 0 .and. FindFunction("VldBemCTE")
		nPosATF := GetPosSD1("D1_CBASEAF")
		if nPosATF > 0
			for nZ := 1 to len(aCols)
				if !empty(aCols[nZ][nPosATF])
					//Função de validação do ativo fixo.
					// .F. =  Ativo não existe ou está classificado.
					// .T. = Ativo existe e está pendente de classificação, podendo ser alterado.
					lContinua := VldBemCTE(left(aCols[nZ][nPosATF],TamSx3("N1_CBASE")[1]),right(aCols[nZ][nPosATF],TamSx3("N1_ITEM")[1]))
					if !lContinua
						Help(" ",1,"A103ATFEXC",,STR0580 + Alltrim(aCols[nZ][nPosATF]) + STR0581,1,0) //"O status do ativo " # " " vinculado a este documento, não permite a exclusão. Verifique o status do ativo"."
					endif
				endif
			next nZ
		endif
	...
	If (l103Inclui .Or. l103Class) .And. !l103Auto
		...
		If lContinua .and. lIntGC // Modulos do DMS
			If FindFunction("OA2900011_A103NFiscal_PodeClassificar")
				lContinua := OA2900011_A103NFiscal_PodeClassificar( { cTipo , cNFiscal , INCLUI } ) // Verifica se é possivel classificar a NF pelo MATA103
			EndIf
		EndIf
	...
	If lContinua
		...
		If ( l103Auto )
			...
			If l103Inclui
				...
				If cPaisLoc == "BRA"
					...
					//Danfe
						...
						If ProcH("F1_CHVNFE") > 0
							If !l103GAuto .or. ( FwIsInCallStack("VEIXX000") .And. aAutoCab[ProcH("F1_ESPECIE"),2] == "NFA" .And. Empty(aAutoCab[ProcH("F1_CHVNFE"),2]) )	// Nao deve efetuar validacao da chave na importacao do XML ou no vinculo de pedidos de compra do TOTVS Colab (COMXCOL)
				...
				If nOpc == 1 .And. cFormul == "S" .And. !Empty(cSerie) .And. cTipoNf <> "3" .And. len(FWGetSX5('01',ALLTRIM(cSerie))) > 0
					If Empty(cNFiscal) //Quando não enviar o F1_DOC, somente a serie
						cNFiscal := FWGetSX5("01",cSerie)[1][4]
					Endif
					NxtSX5Nota( cSerie, .T., cTipoNf, , , , cSerie, .T. )
				Endif
			...
			If nOpc == 1 .Or. lWhenGet
				...
				For nX := 1 To Len(aAutoItens)
					...
					EndIf
					If nY > 0
						lTesTrfOk := A103TrfIPI(aAutoItens[nX, nY, 2], nX)
					EndIf
				Next nX
		...
		If (!l103Auto .Or. lWhenGet) .And. lProcGet
			...
			If cPaisLoc == "BRA"
				If l103Visual .AND. !l103Exclui .AND. !Empty(SF1->F1_STATUS) .And. ( ;
                    ( SA2->A2_CONFFIS <> "3" .And. ;// Diferente de '3 - nao utiliza'
                        ...
                        (cTipo == "B" .And. cMVCONFFIS == "S" .And. cMVTPCONFF == "2") ) .Or. ;
                    ( cTipo == "D" .And. cMVCONFFIS == "S" .And. cMVTPCONFF == "2") ) 
			...
			If !l103Inclui .And. FindFunction("gatilhadkd") 
				gatilhadkd()  
			Endif
		...
		ElseIf EMPTY(cNFiscal) .And. lTesTrfOk
		...
		If nOpc == 1 .And. (l103Inclui.Or.l103Class.Or.l103Exclui)	.And. If(Type("l103GAuto") == "U" ,.T.,l103GAuto)
							...
							If __lIntPFS .And. FindFunction("JURA281") .And. !lEstNfClass
								...
								If lRet
									If l103Exclui .And. FindFunction("J281ExcDoc")
										J281ExcDoc() // Integração com SIGAPFS no momento da exclusão do documento de entrada
									EndIf
								Else
									DisarmTransaction()
								EndIf
			...
			Else
				...
				//Limpa o cache do modelo se o usuário clicou no botão Cancelar no Documento de Entrada
...
//Métricas - Tempo de Inclusão do documento de entrada
...
If lRet .And. nOpc <> 0 .And. FindFunction('JobMultATF') .And. !l103visual .And. SF1->F1_STATUS == 'A'
	...
	cA100For 	:= PADR(cA100For,TAMSX3("F1_FORNECE")[1]," ")
	cLoja 		:= PADR(cLoja,TAMSX3("F1_LOJA")[1]," ")
	cTipo 		:= PADR(cTipo,TAMSX3("F1_TIPO")[1]," ")
...
/*/{Protheus.doc} ProcH
Busca campo array cabeçalho execauto
...
Local cItemIt   	:= ""
Local cMsgDHR		:= ""
Local lObgNatRen	:= X3Obrigat("DHR_NATREN")
local lActForDif    := .T.
local lEntrTer      := SuperGetMV("MV_FORPCNF",.F.,.F.) //Entrega por terceiro?
...
If lRet .And. cPaisLoc == "BRA" .And. Type("lIntermed") == "L" .And. lIntermed .And. cFormul == "S" .And. !l103Class .And. lDtNT2006 
	If SubStr(aInfAdic[16],1,1) $ "0|5" .And. !Empty(aInfAdic[17])
		Help(" ",1,"A103INTA1U",,STR0536 + RetTitle("F1_CODA1U"),1,0) //"Não é necessario informar um "
		lRet := .F.    
	Endif
Endif
...
If lRet
	For nx:=1 to len(aCols)
		...
		If lRet .And. !aCols[nx][Len(aCols[nx])]
			...
			//Verifica se documento gera duplicata (Utilizado na query após o laço do Acols)
				...
				If SC7->(dbSeek(xFilEnt(cFilSC7, "SC7")+aCols[nx,nPosPc]+aCols[nx,nPosItPc]))
					While SC7->C7_FILENT == xFilEnt(cFilSC7, "SC7") .And. SC7->C7_NUM == aCols[nx,nPosPc] .And. SC7->C7_ITEM == aCols[nx,nPosItPc]
						If SC7->C7_FILIAL == cFilSC7 .And. !(SC7->C7_CONAPRO $ 'L ')								
							if !lEntrTer 
								lActForDif := SC7->C7_FORNECE == cA100For
							endif
		...
		If lRet .And. ChkFile("DHR") .And. Len(aColsDHR) > 0 .And. (nA	:= aScan(aColsDHR,{|x| x[1] == aCols[nX,nPosItem]})) > 0
			If Empty(aColsDHR[nA,2,1,1]) .And. lObgNatRen
				cMsgDHR += Iif(Empty(cMsgDHR),STR0577 + AllTrim(aCols[nX,nPosItem])," | " + AllTrim(aCols[nX,nPosItem])) //"O(s) item(ns): "
				lRet := .F.
			Endif
		Endif

		//Validações - SEFAZ AM
		If lRet .and. lCsdXML .and. ( l103Class .Or. INCLUI ) .and. (cTipo == "N" .or. cTipo == "D") .and. Alltrim(cEspecie) $ "NFE|SPED"
		...
		if lRet .and. lCsdXML .and. ( l103Class .Or. INCLUI ) .and. (cTipo == "N" .or. cTipo == "D") .and. Alltrim(cEspecie) $ "NFE|SPED" .And. !(AllTrim(aCols[nX,nPosCfo]) $ cExcCFOP)

			if len(aAutoCSD) > 0
				for nY := 1 to len(aAutoCSD)
					if aScan(aAutoCSD[nY], {|x|AllTrim(x[1])== "DKA_ITXML" }) == 0
						//"Há divergências nos itens da NF - O envio do campo DKA_ITXML é obrigatório quando o parâmetro MV_CSDXML está ativo."
					...
					if lRet .and. aScan(aAutoCSD[nY], {|x|AllTrim(x[1])== "DKA_DESCFO" }) == 0 .and. empty(GetAdvFVal("SA5","A5_DESCPRF",fwxFilial("SA5") + ca100for + cLoja + aCols[nX][nPosCod] ,1))
						//"Há divergências nos itens da NF - É obrigatório o envio do campo DKA_DESCFO - Descrição do fornecedor, quando não houver uma descrição no produto x fornecedor."
					...
					if lRet
						...
						if nPosAXml == 0
							//"Há divergências nos itens da NF - É obrigatório o envio do campo DKA_QTDXML - Quantidade do XML."
							Help(,, "A103ITXML",, STR0554 , 1, 0,,,,,,)
							lRet := .F.
							Exit
						elseif nPosAXml > 0
							if aAutoCSD[nY][nPosAXml][2] <= 0
								//"Há divergências nos itens da NF - O valor da quantidade do XML deve ser maior que zero."
				...
				if lRet .and. aScan(aAutoCSD, {|x|AllTrim(x[1,2])== aCols[nX][nPosItXML] }) == 0
					//"Há divergências nos itens da NF - Foi enviado um item XML(DKA_ITXML) que não existe no D1_ITXML dos itens da NF"
			...
			if empty(Alltrim(aCols[nX][nPosItXML])) .and. !aCols[nX][Len(aCols[nX])]
				//"Há divergências no item " # "Quando o parâmetro MV_CSDXML estiver ativo, torna-se obrigatório o preenchimento do campo Item XML"
				...
				if nPosAXml == 0
					...
					if Alltrim(aItensDKA[nPosAXml][1]+aItensDKA[nPosAXml][2]+aItensDKA[nPosAXml][3]) <> Alltrim(StrZero(Val(aCols[nX][nPosItXML]), TamSX3("D1_ITXML")[1])+aCols[nX][nPosTes]+aCols[nX][nPosCod])
						//"Há divergências no item XML " # "Os produtos e TES devem ser iguais quando são parte do mesmo Item do XML"
...
If !lRet .And. !Empty(cMsgDHR) 
	cMsgDHR += STR0578 //" não tiveram a natureza de rendimento preenchida"
	Help( " ", 1, "A103DHROBRIG", , cMsgDHR, 1, 0 ) //Os itens XXXX | XXXX não tiveram a natren preenchida
Endif
...
If lRet .And. Empty(ca100For) .Or. Empty(dDEmissao) .Or. Empty(cTipo);
		.Or. (Empty(cNFiscal) .and. cFormul <>  "S"); //TELA - Nota fiscal em branco e formulário proprio diferente de SIM
		.Or. ( (Empty(cNFiscal) .Or. Empty(cSerie)) .And. cFormul == "S" .And. l103Auto .And. !lAbreTela);//EXECAUTO - Nota e Serie em branco, formul prop = SIM e execauto não abre tela
		.Or. (lEspObg .And. Empty(cEspecie) .And. l103Auto .And. !lAbreTela); //EXECAUTO - Especie em branco e não abre tela para edição
		.Or. (lEspObg .And. Empty(cEspecie) .And. !l103Auto) //TELA - Especie em Branco
	Help(" ",1,"A100FALTA")
	lRet := .F.
EndIf
...
If lRet .And. lContinua
	...
	If lRet .And. alltrim(cEspecie) $ "SPED|CTE|CTEOS|NF3E"
		DbSelectArea("SX3")
		DbSetOrder(2)
		If MsSeek("F1_CHVNFE")
		...
		If lRet .And. lVerChv .And. cFormul == "N" .And. !Empty(aNfeDanfe[13])
			...
			If !Empty(cSerie)
				...
				//Tratamento para verificar se a série informada é alfanumerica
				...
				If ( cCGC == cNFForn .Or. lAvulsa ) .And. Val(cNFiscal) == nNFNota .And. (Val(cSerie) == nNFSerie) .And. !lDif .Or. Existblock("M103ALTS")
					lRet := .T.
				Elseif (cTipo == 'B' .Or. cTipo == 'D') .And. ( cCGC == cNFForn .Or. lAvulsa ) .And. Val(cNFiscal) == nNFNota .And. (Val(cSerie) == nNFSerie) .And. !lDif // tratamento para beneficiamento e devolução
					lRet := .T.
				Else
...
If lRet .And. lCsdXML .And. (cTipo == "N" .or. cTipo == "D") .and. Alltrim(cEspecie) $ "NFE|SPED"
	lExcCsd := .F. //-- Flag de exceção do consolidador 
	If nItensExc > 0 .And. nTotItens <> nItensExc .And. !Empty(cItemIt)//Nem todos itens são exceção (com isso todos itens precisam ser preenchidos)
		//"Há divergências nos itens " # "Quando o parâmetro MV_CSDXML estiver ativo, torna-se obrigatório o preenchimento do campo Item XML"
		Help(,, "A103ITXMLCFOP",, STR0572 + SubStr(cItemIt,2) , 1, 0,,,,,,{STR0573}) //"Há divergências nos itens " ## "Com o MV_CSDXML ativo, torna-se obrigatório o preenchimento de todos os campos Item XML, caso houver CFOP fora do parametro (MV_EXCCSD) de exceção."
		lRet := .F.
	Elseif nItensExc > 0 .And. nTotItens == nItensExc//Somente exceções estão no documento.
		lExcCsd := .T.
	Endif
Endif
...
if lRet .and. !lExcCsd .and. lCsdXML .And. (cTipo == "N" .or. cTipo == "D") .and. Alltrim(cEspecie) $ "NFE|SPED"
...
If CheckCols(n,aCols)
	...
	If !aCols[n][Len(aCols[n])]
		...
		if lCsdXML .and. (cTipo == "N" .or. cTipo == "D") .and. Alltrim(cEspecie) $ "NFE|SPED" .And. !(AllTrim(aCols[n,nPosCfo]) $ cExcCFOP)
			if !D3Q->(MsSeek(fwxFilial("D3Q") + aCols[n][nPosCod]))
				lRet := .F.
				Help(" ",1,"CSDCONV",,STR0548,1,0)//"Não há cadastro de conversão para este produto. O cadastro é obrigatório quando MV_CSDXML = .T."
			endif
		endif
		...
		//Valida ativo digitado em NF de garantia estendida. (Agrega valor no bem)
		if lRet .and. nPosATF > 0 .and. cTipo == "N" .and. (INCLUI .or. l103Class .or. (!INCLUI .and. !ALTERA)) .and. FindFunction("VldBemCTE")
			if !empty(aCols[n][nPosATF])
				...
				if !lRet
					Help(" ",1,"A103VLDATF",,STR0567 + Alltrim(aCols[n][nPosATF]) + STR0568,1,0) //"O ativo " # " não existe ou possui status que não permite alteração."
				endif
			endif
		endif
		...
		If lRet
			...
			Case nPosPC > 0 .and. nPosItemPC > 0 .and. !Empty(aCols[n][nPosPc]) .And. !Empty(aCols[n][nPosItemPC])
				...
				if(Alltrim(aCols[n][nPosCod]) <> Alltrim(GetAdvFVal("SC7","C7_PRODUTO",xFilEnt(fwxFilial("SC7"),"SC7") + aCols[n][nPosCod] + aCols[n][nPosPc] + aCols[n][nPosItemPC],19)))
					//--Não é permitido a alteração de produto vinculado a pedido de compra. Verifique a linha: # Desfaça a alteração do produto ou delete a linha alterada.
					Help(,,"A103PCPROD",,STR0575 + aCols[n][nPosItemNF], 1, 0,,,,,,{STR0576})
					lRet := .F.
				ElseIf !CtbAmarra(Iif(nPosConta >0,aCols[n,nPosConta],''),Iif(nPosCC >0,aCols[n,nPosCC],''),Iif(nPosItemCTA >0, aCols[n,nPosItemCTA],''),Iif(nPosCLVL >0, aCols[n,nPosCLVL],''), /*lPosiciona*/,/*lHelp*/,/*lValidLinOk*/,aEntid2)
					lRet := .F.
				endif
		...
		If nPosNFOri>0 .And. nPosSerOri>0 .And. nPosCod>0 .And. nPosItmOri>0 .And. nPosQuant>0 .And. nPosTotal>0 .And.;
			...
			If MsSeek(xFilial("SD2", cFilOri)+aCols[n][nPosNFOri]+aCols[n][nPosSerOri]+SF2->F2_CLIENTE+SF2->F2_LOJA+aCols[n][nPosCod]+aCols[n][nPosItmOri])
				...
				If cVldPDev == "U" 	//-- Valida pelo preco unitario (devera ser igual)
					nVlUnACols := a410Arred(aCols[n,nPosVUnit],"D1_VUNIT") //Arredonda o valor total de acordo com as casas decimais, ao utilizar a opção retornar
				...
				//Validacao da NF de Origem Mais Negócios
...
If lRet .And. lRevProd
	If nRevisao > 0
		...
		EndIf
	Else
		help( NIL, 1, "RevEstrut", NIL, STR0569, 1 , 0,,,,,, {chr(13) + chr(10) + STR0570 +chr(13) + chr(10) +STR0571}) // "Não é possivel verificar a revisão da estrutura." , "- Deixar o campo 'D1_REVISAO' como usado no configurador;" "- Verificar o parâmetro MV_REVPROD."
		lRet:= .F.
	Endif		
EndIf

if lRet .and. lAgVlrATF .and. cTipo == "N" .and. Alltrim(cEspecie) == "CTE" .and. nPosNFOri > 0 .and. nPosSerOri > 0 .and. nPosItmOri > 0 .and. SF4->F4_ATUATF == "S"
	if empty(aCols[n][nPosNFOri]) .or. empty(aCols[n][nPosSerOri]) .or. empty(aCols[n][nPosItmOri]) 
		lRet := .F.
		Help(" ",1,"A103VLDATF",,STR0582,1,0) //"Para utilizar TES que atualiza ativo é obrigatório o preenchimento do documento, série e item de origem no CTE."
	else
		if SD1->(Msseek(fwxFilial("SD1") + aCols[n][nPosNFOri] + aCols[n][nPosSerOri] + ca100for + cloja))
			While SD1->(!Eof()) .and. fwxFilial("SD1") + aCols[n][nPosNFOri] + aCols[n][nPosSerOri] + ca100for + cloja == SD1->(D1_FILIAL + D1_DOC + D1_SERIE + D1_FORNECE + D1_LOJA)
				if SD1->D1_ITEM == StrZero(val(aCols[n][nPosItmOri]),len(SD1->D1_ITEM)) .and. empty(SD1->D1_CBASEAF)
					lRet := .F.
					Help(" ",1,"A103VLDATF",,STR0583,1,0) //"Para utilizar TES que atualiza ativo é obrigatório o preenchimento de um documento, série e item que possua ativo a ser agregado valor."
					Exit
				elseif SD1->D1_ITEM == StrZero(val(aCols[n][nPosItmOri]),len(SD1->D1_ITEM)) .and. !empty(SD1->D1_CBASEAF) .and. FindFunction("VldBemCTE")
					if !VldBemCTE(left(SD1->D1_CBASEAF,TamSx3("N1_CBASE")[1]),right(SD1->D1_CBASEAF,TamSx3("N1_ITEM")[1]))
						lRet := .F.
						Help(" ",1,"A103VLDATF",,STR0567 + Alltrim(SD1->D1_CBASEAF) + STR0584,1,0)//"O ativo ###  vinculado ao documento de origem, possui status que não permite agregar valor."
						Exit
					endif
				endif
				SD1->(DbSkip())
...
//Verifica se está permitindo salvar está permitindo salvar centro de custo no produto com rateio em caso do parâmetro MV_M103LRA = .T.
If lRet .And. aCols[n][nPosRateio]=="1" .And. lM103lRat .And. ;
    (!EMPTY(aCols[n][nPosCC]) .Or. !EMPTY(aCols[n][nPosConta]) .Or. !EMPTY(aCols[n][nPosItemCTA]) .Or. !EMPTY(aCols[n][nPosCLVL]))
    Help('   ',1,'A103RATEI')
    lRet:= .F.
EndIf
...
Local nQtdAglut			:= 1//Qtd de produtos iguais q foram aglutinados no mata116.
Local cItemPrd			:= ""
Local nVlrRat 			:= 0
Local nVlrIcm 			:= 0
Local cCodProd 			:= ""
...
Local aAuxAtu			:= {}
Local aAuxICM			:= {}
Local aAtuBens			:= {} //Array de atualização de bens.
Local cOpc 				:= "1"//Opção --> 1 = Atualiza os valores / 2 = Estorno de valores.
Local cDocOri			:= ""
Local cSerOri 			:= ""
...
ElseIf nOpc == 102 .And.  lAjustaNCD .And. lStrutNCD
	...
	Elseif nOpc == 104 //Realiza atualização de valor dos bens (Exemplo: Conhecimento de frete agrega valor ao bem)

	nVlrAdc   := Eval( bValAtf )
	nVlrAdc   := IIf(ValType(nVlrAdc) <> "N",(SD1->D1_TOTAL-SD1->D1_VALDESC)+IIf(SF4->F4_CREDIPI=="S",0,SD1->D1_VALIPI)-IIf(SF4->F4_CREDICM=="S",SD1->D1_VALICM,0),nVlrAdc)
	...
	if empty(aRecSF1Ori) .and. ( FwIsInCallStack("MATA103") .OR. FwIsInCallStack("A140ESTCLA") )
		SF1->(DbSetOrder(1))
		...
		if !empty(cBaseDigit)
			aAdd(aRecSF1Ori,SF1->(Recno())) //Guardo o recno da propria NF, pois não existe origem.

			//Cliente pode enviar somente a base do ativo para atualizar todos os itens.
			//Ou pode enviar a base + item para atualizar valor de um item específico.
	...
	for nX := 1 to len(aRecSF1Ori)
		...
		//cBaseDigit em preenchida = NF de garantia estendida, já está posicionado na SF1 correta.
		//cBaseDigit em branco = Inclusão de CTE, requer posicionamento na SF1 de origem.
		if empty(cBaseDigit)
			SF1->(DbGoTo(aRecSF1Ori[nX]))
			cChaveSD1 := SF1->(F1_FILIAL + F1_DOC + F1_SERIE + F1_FORNECE + F1_LOJA)
		endif

		if FwIsInCallStack("MATA116") .and. type("lAglutProd") == "L" .and. lAglutProd 
			nQtdAglut := getProdIguais(SF1->F1_DOC,SF1->F1_SERIE,SF1->F1_FORNECE,SF1->F1_LOJA,cCodProd)
			nVlrAdc := Round(nVlrAdc/nQtdAglut,2)
		endif
		...
		if SD1->(MsSeek(cChaveSD1))
			while SD1->(!eof()) .and. SF1->(F1_FILIAL + F1_DOC + F1_SERIE + F1_FORNECE + F1_LOJA) == SD1->(D1_FILIAL + D1_DOC + D1_SERIE + D1_FORNECE + D1_LOJA)
				if !empty(cDocOri) .and. !empty(cSerOri) //Verifica se o item a ser agregado pertence a origem. Mata116 permite selecionar mais de um doc.
					if cDocOri + cSerOri <> Alltrim(SD1->D1_DOC) + Alltrim(SD1->D1_SERIE)
						Exit
					endif
				endif
				if !empty(cItemPrd) //Se possuir o D1_ITEMORI, utiliza para posicionar no item correto(D1_ITEMORI é preenchido no MATA116 AglutProd = Não e Comp Frete via MATA103).
				...
				if !empty(SD1->D1_CBASEAF) .And. cCodProd == AllTrim(SD1->D1_COD)
					...
					if !empty(cBaseSD1) .and. !empty(cItBsSD1) .and. val(cItBsSD1) > 0
						...
						aRetAtf := CompCTE(aAtuBens,cOpc)//(Função do ativo) -- Envia os dados para o ativo fixo atualizar os bens.
...
Else
	...
	If ( !Empty(cBase) ) .and. !isNfGatEst(SD1->D1_DOC,SD1->D1_SERIE,SD1->D1_FORNECE,SD1->D1_LOJA)
		DbSelectArea("SN1")
		DbSetOrder(1)
		cBase := Alltrim(cBase)
...
//Proteção para quando o a103grava for chamado por outro fonte.
If Type("l103Class") <> "L"
	l103Class := .F.
Endif
If Type("l103Auto") <> "L"
...
//Metricas documento de entrada com integração com ativo fixo
If lGravou
	nQtdAtf++
...
If SF1->( FieldPos( "F1_GF" ) ) > 0
	aAdd(aLegenda, {"BR_MARRON_OCEAN",STR0562}) //"NF Não Classificada com GF"
EndIf
			...
			If !lMt103FDV .Or. ExecBlock("MT103FDV",.F.,.F.,{cAliasSD2})
				...
				If nSldDev > 0 .Or. (cTipoNF$"CIP" .And. (cAliasSD2)->D2_VALDEV == 0) .Or.;
				   ( (cAliasSD2)->D2_QUANT == 0 .And. (cAliasSD2)->D2_VALDEV == 0 .And. (cAliasSD2)->D2_TOTAL > 0 ) .Or.;
					...
					If nPosEC05CR > 0 .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC05CR ))
                        AAdd( aLinha, { "D1_EC05CR"  , (cAliasSD2)->D2_EC05CR  , Nil } )
                    EndIf
                    
					If nPosEC05DB > 0 .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC05DB ))
                        AAdd( aLinha, { "D1_EC05DB"  , (cAliasSD2)->D2_EC05DB  , Nil } )
                    EndIf
                    
					If nPosEC06CR > 0  .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC06CR ))
                        AAdd( aLinha, { "D1_EC06CR"  , (cAliasSD2)->D2_EC06CR  , Nil } )
                    EndIf
                    
					If nPosEC06DB > 0  .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC06DB ))
                        AAdd( aLinha, { "D1_EC06DB"  , (cAliasSD2)->D2_EC06DB  , Nil } )
                    EndIf
                    
					If nPosEC07CR > 0  .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC07CR ))
                        AAdd( aLinha, { "D1_EC07CR"  , (cAliasSD2)->D2_EC07CR  , Nil } )
                    EndIf
                    
					If nPosEC07DB > 0  .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC07DB ))
                        AAdd( aLinha, { "D1_EC07DB"  , (cAliasSD2)->D2_EC07DB  , Nil } )
                    EndIf
                    
					If nPosEC08CR > 0  .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC08CR ))
                        AAdd( aLinha, { "D1_EC08CR"  , (cAliasSD2)->D2_EC08CR  , Nil } )
                    EndIf
                    
					If nPosEC08DB > 0 .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC08DB ))
                        AAdd( aLinha, { "D1_EC08DB"  , (cAliasSD2)->D2_EC08DB  , Nil } )
                    EndIf
                    
					If nPosEC09CR > 0 .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC09CR ))
                        AAdd( aLinha, { "D1_EC09CR"  , (cAliasSD2)->D2_EC09CR  , Nil } )
                    EndIf
                    
					If nPosEC09DB > 0 .And. !(Empty((cAliasSD2)->D2_EC05CR )) .And. !(Empty((cAliasSD2)->D2_EC09DB ))
                        AAdd( aLinha, { "D1_EC09DB"  , (cAliasSD2)->D2_EC09DB  , Nil } )
                    EndIf
...
If nOpcA == 1
    ...
    // ------------------------------------------------------------------------------
    // Revisão do Rateio do IRRF/PCC
    // Sistema verifica os parâmetros MV_RATPIS - MV_RATCOF - MV_RATCSLL - MV_RATIRRF
    // Porém, o utilizador pode DIGITAR ou acionar o BOTÃO RAT.IMP. alterando a regra
    // ------------------------------------------------------------------------------
    IF LEN(aColsSE2) > 1
        AFILL(aCount,0)
		...
		//-- Fornecedor tipo "F" não carrega os impostos na grid para IR na baixa
	...
	For nX := 1 To nMaxFor
	    If aColsSE2[nX][nPValor] > 0
			...
			If lIntALC
				cAprov := FA050Aprov(nMoedaCor) //verifica se há aprovador com os parâmetros novos MV_FINAPXX
				If Empty(cAprov)//caso não estiver configurado, verifica o parâmetro antigo
					cAprov := cMVFinAlap
				Endif
			Endif
			...
			//Valida se o codigo de retenção informado aplica o rateio de IR Aluguel 
			If oRatIRF <> NIL
				If Alltrim(cCodRet) $ cCdRetIRRt
					If !Empty(oRatIRF:aRatIRF) .And. Len(oRatIRF:aRatIRF) > 1
						...
						Else
							...
							nIRRateio += nValIrrf
							//Proporcionalização entre as duplicatas
							For nY := 1 to Len(oRatIRF:aRatIRF)
			...
			//função para verificar os ids de impostos do configurador de tributos para barrar a geração do título pelo padrão.
			...
			If cPaisLoc == "BRA"
				...
				If lVisDirf
					If lHasIRR .And. cDirf=="1" .And. SE2->E2_BASEIRF>0//-- Se houve retenção do IR em alguma parcela, mantem o código dele.
						SE2->E2_CODRET := aCodR[aScan( aCodR, {|aX|aX[4]=="IRR"})][2]
					ElseIf SE2->(E2_BASEIRF+E2_BASECOF+E2_BASEPIS+E2_BASECSL+E2_BASEISS)<=0
						SE2->E2_CODRET :=""
					EndIF
					//-- Não atualiza antes pois FaAvalSE2/A050DupPag olha o DIRF='1' para gerar o registro TX 
					SE2->E2_DIRF := "2"
				EndIF
            EndIf
	...
	//Variaveis para gravação do campo E2_TITPAI
	...
	IF MV_PAR23 == 1 .And. !Empty(cNatPis) .And. !Empty(cNatCof) .And. lPisCofImp
		...
		If SF1->F1_VALIMP6 > 0
			...
			aTit050 := {{"E2_FILIAL", xFilial("SE2"), NIL},;
							...
							//Adicionado E2_TITPAI, para que não se perca o vinculo com o titulo pai (nota de entrada). 
...
Local aD1Area		:= {}
Local lOrigGC		:= .F.
Local lFuncGC       := ( FindFunction("OA2900051_a103Grava_SQL_SE1") .and. FindFunction("OA2900061_a103Grava_ver_geracao_SE1") ) // Funções relacionadas ao SE1 (TIPO)
Local nRecSE1       := 0
Local nValIcmcom 	:= 0
Local lDifTit		:= .F.
Local lCompDifal 	:= .F.
...
If lContinua
	...
	If !lDeleta
        ...
        If l103Auto
			For nX := 1 To Len(aAutoCab)	
				If SF1->(FieldPos(aAutoCab[nX][1])) > 0
		...
		//Ponto de entrada para atualizações na SF1 após finalização da gravação da SF1
		...
		For nX := 1 to Len(aCols)
			...
			If !aCols[nx][Len(aHeader)+1]
				...
				// Grava array aOPBenef na Classificação após Bloqueio por Tolerancia
				...
				//-- Avaliar a existencia de ao menos um vínculo de SC x PC x NF 
				If !lSCxPCxNF
				...
				If l103ATURM //Atualiza saldo no PC (Retenção/Dedução/Faturamento Direto)
				...
				If FwIsInCallStack("MATA116") .and. Type("l116Auto") == "L" .and. l116Auto
					nPosPedg := aScan(aAutoCab,{|x| x[1] == "F1_VALPEDG"})
					If nPosPedg > 0
						MaFisAlt("NF_VALPEDG",SF1->F1_VALPEDG)  
						SF4->(MaFisWrite(2,"SF1",Nil))
					Endif
				Endif
				...
				If cF4FRETISS <> "2" //Verifica se a TES usa o valor Mínimo
				...
				If SF1->F1_STATUS == "A" //Classificada sem bloqueio (NORMAL)
					...
					If ( SF4->F4_ATUATF=="S" ) .And. !(SF1->F1_TIPO $ "I|P")
						...
						if ( ( (Alltrim(cEspecie) == "CTE" .and. lAgVlrATF) .OR. ( !empty(SD1->D1_CBASEAF) .and. cTipo == "N") ) ) .and. ( FwIsInCallStack("MATA103") .OR. FwIsInCallStack("MATA116") )
							...
							EndIf
							lContinua := a103GrvAtf(104,,,,,,,,,,aRecSF1Ori,cChaveSD1,SD1->D1_CBASEAF)//Atualização de valor dos bens.
						else
				...
				If lIntGC
					...
					DbSetOrder(2)
					MsSeek(xFilial("SF2")+SF1->F1_FORNECE+SF1->F1_LOJA+SD1->D1_NFORI+SD1->D1_SERIORI)
					lOrigGC := ( SF2->F2_PREFORI $ cPrefBAL .Or. SF2->F2_PREFORI $ cPrefOFI .Or. SF2->F2_PREFORI $ cPrefVEI )
				EndIf	
		...
		If SF1->F1_STATUS == "A" //Classificada sem bloqueio
			...
			If cTipo == "D" .And. MaFisRet(,"NF_BASEDUP") > 0 .And. (!lIntTMS .Or. !ExistFunc("TMSA500NCC") .Or. TMSA500NCC())
				...
				If SuperGetMv( "MV_TXMOENC" ) == "2" .Or. lMoedTit
					...
					aadd(aTxMoeda,{nMoedaCor, nTaxaNCC})
				Else
					nTaxaNCC := 0
				EndIf
				...
				cQuery    += IIf( lIntGC .and. lOrigGC .and. lFuncGC , OA2900051_a103Grava_SQL_SE1() , " AND SE1.E1_TIPO = 'NF '" ) // Módulos de Concessionárias também considera outros Tipos de Titulos
				...
				While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
					...
					nRecSE1 := IIf( lQuery , (cAliasSE1)->SE1RECNO , (cAliasSE1)->(RecNo()) )
					If lCompensa
						If iIf( lIntGC .and. lOrigGC .and. lFuncGC , OA2900061_a103Grava_ver_geracao_SE1( nRecSE1 ) , (cAliasSE1)->E1_TIPO == "NF ") .And. (cAliasSE1)->E1_SITUACA == "0"
							If !SuperGetMv("MV_CHECKNF",.F.,.F.)
								aadd(aRecSE1,nRecSE1)
								If lMoedTit
						...
						Endif
					Endif
					AtuSldNat((cAliasSE1)->E1_NATUREZ,(cAliasSE1)->E1_VENCREA,(cAliasSE1)->E1_MOEDA,"2","R",(cAliasSE1)->E1_VALOR,(cAliasSE1)->E1_VLCRUZ, "-",,cFunName,"SE1",nRecSE1,Iif(INCLUI,3,4))
				...
				If lCompensa
					If nTotalDev > 0
						If lCtbOnLine
							...
							MaIntBxCR(3,aRecSe1,,aRecNcc,,{lCtbOnLine,.F.,.F.,.F.,.F.,.T.},,,,,,,,,nTaxaNCC,aTxMoeda, MV_PAR08 == 1 )
							Pergunte("MTA103",.F.)
	...
	Else
		...
		//Contabilização da LP 65D - Cancelamento de rateio de multipla natureza.
			...
			If ( SF4->F4_BENSATF == "1" ) .And. SD1->D1_QUANT >= 1 .And. !(SF1->F1_TIPO $ "I|P")
				...
				Else
					...
					elseif !empty(SD1->D1_CBASEAF) .and. cTipo == "N" .and. isNfGatEst(SD1->D1_DOC,SD1->D1_SERIE,SD1->D1_FORNECE,SD1->D1_LOJA)
						cChaveSD1 := if(Alltrim(cEspecie) == "CTE",SD1->(D1_FILIAL+D1_NFORI+D1_SERIORI+D1_FORNECE+D1_LOJA),SD1->(D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA))
						a103GrvAtf(104,,,,,,,,,,aRecSF1Ori,cChaveSD1,SD1->D1_CBASEAF)//estorna atualização de valor dos bens.
					EndIf
				EndIf
			ElseIf !(SF1->F1_TIPO $ "I|P")
				if ( ( ( Alltrim(cEspecie) == "CTE" .and. lAgVlrATF .and. SF4->F4_ATUATF == "S" ) .OR. ( !empty(SD1->D1_CBASEAF) .and. cTipo == "N" .and. isNfGatEst(SD1->D1_DOC,SD1->D1_SERIE,SD1->D1_FORNECE,SD1->D1_LOJA)) ) ) ;
						.and. ( FwIsInCallStack("MATA103") .OR. FwIsInCallStack("MATA116") .OR. FwIsInCallStack("A140ESTCLA") )
...
if (INCLUI .Or. l103Class) .and. lCsdXML .and. Type("oMdlCSDGRV") == "O" 
	...
	if(cFormul == "S") //Formulário próprio, possuo o num do doc só no final do processo.
...
/*/{Protheus.doc} NfeCalcRet
	...
	@since 24/03/2021
	@version 1.0
	@param dReferencia, Date, Data de referencia para verificação dos titulos
	@param nIndexSE2, Numeric, Indice da tabela SE2
	@param aDadosImp, Array, Array que ira conter as informações das retenções
	@return aDadosRef, Array , Array que ira conter as informações das retenções
...
If l103Class 
	...
	lExistSCR1 := MtExistSCR("NF", cKeySF1, .F.) //-- Controla se existe SCR nao deletada para cenários de classificação de NF que teve alterações na Guarda Fiscal
	lExistSCR2 := MtExistSCR("NF", cKeySF1, .T.) //-- Controla se existe SCR deletada para cenários de reclassificação de NF pós estorno
	lLibSCROk  := MtGLastSCR("NF", cKeySF1) //-- Controla se o documento encontra-se totalmente liberado na SCR, para cenários da Guarda Fiscal
	nCRTotal   := GetAdvFval("SCR", "CR_TOTAL", xFilial("SCR") + "NF" + cKeySF1, 1, 0, .T.) //-- Obtém o total do documento na SCR para comparar com o total da SF1 e validar cenários de classificação de NF que teve alterações na Guarda Fiscal
			...
			If !Empty(aCols[nx][nPosPc])
				...
				If lVerifica
					...
					If !Empty(aCols[nx][nPosPc]+aCols[nx][nPosItPc]) .And. MsSeek(cFilEntC7+aCols[nx][nPosCod]+aCols[nx][nPosPc]+aCols[nx][nPosItPc])
						...
						//-- Avalia quebra de 1 item de PC em N itens do Doc. de Entrada vindo de Guarda Fiscal ou Pré-Nota
						nQtAux := 0
						If l103Class
							...
							While SD1->(!EOF()) .And.;
								...
								If SD1->D1_COD == aCols[nX][nPosCod] .And. SD1->D1_PEDIDO == aCols[nX][nPosPc] .And. SD1->D1_ITEMPC == aCols[nX][nPosItPc]
									nQtAux += SD1->D1_QUANT //-- Obtem quantidade que já estava gravada na SD1 para ser avaliada com a quantidade definida no aCols (nQtdItens)
								EndIf
								SD1->(DbSkip())
							EndDo
							RestArea(aAreaAux)
						...
						If lRet
							...
							if !lNfMotBloq//Se o campo na SCR existe, deverá avaliar bloqueio em todos os itens
								Exit
							endif
						EndIf
					EndIf
...
ElseIf SF4->(MsSeek(xFilial("SF4")+cTes)) .And. SF4->F4_TRANFIL == "1" .And. cTipo == "D"
	lRet := .F.
	lTrfil := .F.
	Aviso(STR0119,STR0579,{STR0461},1) //Campo F4_TRANFIL (Transferência Filiais) não pode estar com conteúdo 1=Sim para operações de devolução, utilize um TES com conteúdo 2=Nao
EndIf
...
Else
	...
	While Eval( bWhileSD1 )
		...
		If ( (Empty(cCondicao) .Or. l103Class) .And. !Empty((cAliasSD1)->D1_PEDIDO) )
			...
			SC7->(DbGoTo(nRecC7))
			If nRecC7 > 0
				If !l103Class .Or. (l103Class .And. Empty(SF1->F1_COND))
		...
		For nY := 1 To nUsado
			If ( aHeader[nY][10] <> "V")
				...
				If (l103Class .Or. l103Visual) .And. Alltrim(aHeader[ny][2]) == "D1_TES" .And. Empty((cAliasSD1)->D1_TES)
					...
					Else
						//-- Para Devoluções (D) ou Beneficiamento (B) deve-se avaliar a TES de devolução (F4_TESDV) vinculada na TES do documento de origem (SD2)
						If cTipo $ "D|B" 
							
							//-- Limpar as variáveis de controle a cada iteração do loop
							cTesD2 := "" 
							cTesAux := "" 
							...
							//-- Obtem TES de devolução vinculada a TES utilizada no documento de origem
							If !Empty(cTesD2)
								cTesAux := GetAdvFVal("SF4", "F4_TESDV", cFilSF4 + cTesD2, 1, "", .T.)
							EndIf
							
							//-- Caso ainda assim não encontre, utiliza a do cadastro do produto (SB1)
							If Empty(cTesAux)
			...
			If Alltrim(aHeader[ny][2]) == "D1_COD"
				nPosProd := nY
			EndIf
	...
	For nX := 1 To Len(aCols)
		if l103Class .and. FwAliasInDic("DHR")
			A103NatRen(aHeadDHR,aColsDHR,.T.,.F.,,aCols[nX][nPosProd],aCols[nX][GetPosSD1("D1_ITEM")])
		endif
		If lDivImp
		...
		If SC7->(MsSeek(xFilial("SC7")+aCols[n][nPosPc]+aCols[n][nPosItPc]))			
			...
			ElseIf(cCampo == "M->D1_QUANT")
				...
				If(nTolera > 0)
					...
					If Inclui
						If cPcToSal == "N"
							nQntPedid	:= (SC7->C7_QUANT*nTolera)-SC7->C7_QTDACLA-SC7->C7_QUJE
						Else
							nQntPedid	:= (SC7->C7_QUANT*nTolera)-SC7->C7_QTDACLA
						Endif
					Else	
						DbSelectArea("SD1")
						...
						If cPcToSal == "N"
							nQntPedid	:= (SC7->C7_QUANT*nTolera)-nQuantD1-SC7->C7_QUJE 
						Else
							nQntPedid	:= (SC7->C7_QUANT*nTolera)-nQuantD1
						Endif
					Endif
					...
					If cPcToSal == "N"
						If(nQntNota > nQntPedid)
							Help(' ', 1, 'A103PCNSLD')
							lRet := .F.
						Endif
					Else
						If(nQntNota + SC7->C7_QUJE  > nQntPedid)
							Help(' ', 1, 'A103PCNSLD')
							lRet := .F.
						EndIf	
					Endif
...
If lRet .And. ChkFile("DHR") .And. FwIsInCallStack("MATA103") .And. ; //Vincula natureza rendimento automatica
!(FwIsInCallStack("MATA140") .Or. FwIsInCallStack("MATA116") .Or. FwIsInCallStack("MATA119"))
...
If Type("oLancApICMS")= "O" .Or. lAuto116 .OR. lAuto103 .Or. lAuto920
	...
	Else
		...
		If lAuto116 .OR. lAuto103 .Or. lAuto920
...
Static Function A103Adiant(lUsaAdi)
	...
	If Len(aPedAdt) > 0
		For nCnt := 1 to Len(aPedAdt)
			If lRet 
				...
				For nCnt1 := 1 To Len(aPedidos)
					SC7->(DbSeek(xFilial("SC7")+aPedidos[nCnt1][1])) //Posiciona no pedido de compra com adiantamento, para validar a data de emissão com a data base
		...
		If !lRet 
			Help(,,"A103ADIANT",,STR0545, 1, 0,,,,,,{STR0547}) //"Pedido de compra vinculado ao documento de entrada possuí adiantamento e para que a compensação do título no Financeiro seja realizada corretamente, a data base do sistema não poderá ser retroagida. Ajuste a data base do sistema e faça a inclusão do Documento de Entrada."
		EndIf
		...
		Else
			...
			Else
				If lRet .And. lUsaAdi .And. !lVldAdt .And. !lTemMov //Se o parâmetro MV_VLDADT estiver desativado e não possui movimentação bancaria
...
Function A103DigEnd(aDigEnd)
	...
	If  !lAuto103  .and. MsgYesNo(OemToAnsi(STR0379), OemToAnsi(STR0380))//"Deseja realizar o endereçamento dos itens da nota?", "Endereçar itens"
		//Calcula dimensões
		oSize := FwDefSize():New()
...
Local aRecRet	 := {}
Local nValRetPed := 0
Local lElimRes	 := SuperGetMV("MV_ESLDFIE",.F.,.T.) //Indica se elimina resíduo na tabela FIE (Zerando o FIE_SALDO quando o PC estiver 100% entregue) respeitando o legado
...
If (cTpFrete <> "S") .And. SuperGetMV("MV_INTGFE",.F.,.F.) .And. SuperGetMV("MV_INTGFE2",.F.,"2") $ "1" .And. SuperGetMv("MV_GFEI10",.F.,"2") == "1" .And. (lIsIncl .Or. lIsClass) .And. !(AllTrim(cTipo) $ 'I|P')
	...
	Else
		Help(" ",1,"COMGFE",,STR0542,1,0) //Necessario aplicar expedição continua do GFE para que não haja nenhuma incosistencia na integração junto ao documento de entrada.
		lRet := .F.
	EndIf	
...
If l103Class .And. lMT103MSG
	If  lGuardFis .Or. MsgYesNo(OemToAnsi(STR0454), OemToAnsi(STR0491))//"Deseja salvar as informações inseridas até o momento que não estão relacionadas a cálculo de impostos?", "Salvar Dados"
...
Local lRet := .F.
	...
	If lRet
		...
		If !lRet
			Help(" ",1,"FISXCIAP",,STR0532,1,0) // "Por favor, atualize o fonte FISXCIAP para uma versão igual ou superior a 24/07/2020." 
		EndIf
	EndIf
...
/*/{Protheus.doc} A103CRatIR
	Função responsavel por instanciar o objeto de calculo
...
Quantidade de documento de entrada, por função (A103AtuSE2, A103GrvAtf,MafisAtuSf3)
...
Tempo médio de execução da inclusão do documento de entrada
...
/*/{Protheus.doc} isNfGatEst
	Retorna se é NF de garantia estendida de ativo fixo.
...
If (cAliasSN1)->(!Eof())
	lRet := .F. //a NF gerou ativo de fato. Não é garantia estendida.
Endif
...
/*/{Protheus.doc} getProdIguais
	Retorna a quantidade de produtos iguais na NF de origem
@author Leandro Fini
@since 20/06/2023
/*/
Static Function getProdIguais(cDoc,cSerie,cFornec,cLoja,cProduto)
...
cQuery := "SELECT COUNT(D1_COD) PRODIG FROM " + RetSqlName("SD1")
